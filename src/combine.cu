...
// int tx = threadIdx.x;
// int ty = threadIdx.y;
// if (tx >= TILE || ty >= TILE) return;

// int M = out_shape[1];
// int P = out_shape[2];
// int N = a_shape[2];

// int row = blockIdx.x * TILE + ty;
// int col = blockIdx.y * TILE + tx;
// float acc = 0.0f;

// int num_tiles = (N + TILE - 1) / TILE;

// for (int t = 0; t < num_tiles; t++) {
//   // global indices: load a_shared
//   int a_col = t * TILE + tx;
//   int b_row = t * TILE + ty;
...
for (int i = 0; i < a_shape[reduce_dim]; i++) {
  ...
  reduce_value = fn(fn_id, reduce_value, a_val);
}
out[gid] = reduce_value;


// __shared__ float shared_data[BLOCK_DIM];
// int tid = threadIdx.x;
// int out_gid = blockIdx.x;
// if (out_gid >= out_size) return;

// int a_index[MAX_DIMS];
// to_index(out_gid, out_shape, out_index, shape_size);
...
// // per-thread local reduce over k = tid, tid + blockDim.x, ...
// float acc = reduce_value;
// for (int k = tid; k < reduce_size; k += blockDim.x) {
//   int a_pos = base_pos + k * step;
//   float a_val = a_storage[a_pos];
...
//   if (tid < s) {
//     shared_data[tid] = fn(fn_id, shared_data[tid], shared_data[tid + s]);
//   }
//   __syncthreads();
// }
