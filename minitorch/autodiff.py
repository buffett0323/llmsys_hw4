...
def topological_sort(variable: Variable) -> Iterable[Variable]:
    ...
    def dfs(node):
        ...
        for neighbor in node.parents:
            # constant have no parents
            if neighbor.unique_id not in visited and not neighbor.is_constant():
                dfs(neighbor)
        stack.append(node)
...
def backpropagate(variable: Variable, deriv: Any) -> None:
    ...
    for node in topo_sort:
        ...
        d_out = outgrads.pop(node.unique_id)
        if node.is_leaf():
            node.accumulate_derivative(d_out)
        else:
            for parent, grad in node.chain_rule(d_out):
                if parent.unique_id not in outgrads:
                    outgrads[parent.unique_id] = grad
                else:
                    outgrads[parent.unique_id] += grad
    # END ASSIGN2_1
